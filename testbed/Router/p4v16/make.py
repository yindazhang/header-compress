...
def make_switchd(switchd_in_abs, switchd_out_abs, skip, ether_type, gap, prot):
  ...
  cmd = "g++ -I"+SDE+"pkgsrc/bf-drivers/include -I"+SDE+"install/include -Wno-missing-field-initializers -Werror -Wshadow -g -O2 -std=c++11 -L"+SDE+"install/lib/ -o "+switchd_out_abs+"switchd "+switchd_in_abs+" -ldriver -lbfsys -lbfutils -lbf_switchd_lib -lpcap -lm -ldl -lpthread -pthread -Wl,--disable-new-dtags -L"+SDE+"install/lib -Wl,-rpath -Wl,"+SDE+"install/lib"
  print(cmd)
  subprocess.call(cmd.split())

  cmd = "sudo "+switchd_out_abs+"switchd --kernel-pkt --install-dir "+SDE+"install --conf-file "+switchd_out_abs+"main.conf"
  print(cmd)
  if not skip:
    subprocess.call(cmd.split())

def make_control(switchd_in_abs, switchd_out_abs):
  if os.path.isabs(switchd_in_abs) and os.path.isabs(switchd_out_abs):
    pass
  else:
    print("ERR: non abs path: {}, {}".format(switchd_in_abs, switchd_out_abs))
  ...
  cmd = "g++ -I"+SDE+"pkgsrc/bf-drivers/include -I"+SDE+"install/include -Wno-missing-field-initializers -Werror -Wshadow -g -O2 -std=c++11 -L"+SDE+"install/lib/ -o "+switchd_out_abs+"control "+switchd_in_abs+" -ldriver -lbfsys -lbfutils -lbf_switchd_lib -lm -ldl -lpthread -pthread -Wl,--disable-new-dtags -L"+SDE+"install/lib -Wl,-rpath -Wl,"+SDE+"install/lib"
  print(cmd)
  subprocess.call(cmd.split())

  cmd = "sudo "+switchd_out_abs+"control --kernel-pkt --install-dir "+SDE+"install --conf-file "+switchd_out_abs+"main.conf"
  print(cmd)
  if not skip:
    subprocess.call(cmd.split())
...
if __name__ == '__main__':
  ...
  if args.cmd == "sde":
    ...
    make_control(args.switchd_in, args.switchd_out)
  elif args.cmd == "kill":
    make_kill()